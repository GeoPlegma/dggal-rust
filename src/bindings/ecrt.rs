...
impl Application {
    pub fn new(_args: &Vec<std::string::String>) -> Application {
        unsafe {
            let app = ecrt_sys::ecrt_init(
                nullInst,
                true as u32,
                false as u32,
                0,
                0 /*ptr::null_mut::<* mut * mut i8>()*/ as * mut * mut i8,
            ); // TODO: argc, args);
            Application { app: app }
        }
    }
...
impl TTAU64 for String {
    fn from_u64(value: u64) -> Self {
        String(value as ecrt_sys::String)
    }
    fn to_u64(&self) -> u64 {
        (*self).0 as *mut c_void as u64
    }
}
impl TTAU64 for Instance {
    fn from_u64(value: u64) -> Self {
        Instance(value as ecrt_sys::Instance)
    }
    fn to_u64(&self) -> u64 {
        self.0 as *mut c_void as u64
    }
...
#[macro_export]
macro_rules! delegate_ttau64_and_default {
    ($wrapper:ty) => {
        impl TTAU64 for $wrapper {
            fn from_u64(value: u64) -> Self {
                Self(TTAU64::from_u64(value))
            }
            fn to_u64(&self) -> u64 {
                self.0.to_u64()
            }
    ...
    V: TTAU64 + Copy,
{
    pub fn new() -> Self {
        Self(
            ecrt_sys::MapIterator {
                container: nullInst,
                pointer: nullPtr as IteratorPointer,
            },
            PhantomData,
        )
    }
...
impl<T: Default + TTAU64> Array<T> {
    ...
    pub fn count(&self) -> i32 {
        ...
        if self.array != nullInst {
            unsafe {
                let members: *const ecrt_sys::class_members_Array = ((self.array as *const i8)
    ...
    pub fn getAtPosition(
        ...
        index: i32,
        create: bool,
        justCreated: Option<&mut bool>,
    ) -> IteratorPointer {
        let mut pointer = nullPtr as IteratorPointer;
        unsafe {
            let c = ecrt_sys::class_Array;
            let vTbl = if self.array != nullInst && (*self.array)._vTbl != nullVTbl {
                (*self.array)._vTbl
            } else {
                (*c)._vTbl
            };
            let cMethod: usize =
                std::mem::transmute(*vTbl.add(ecrt_sys::Container_getAtPosition_vTblID as usize));
            if cMethod != 0usize {
                let method: unsafe extern "C" fn(
                    container: ecrt_sys::Array,
                    pos: i32,
                    create: u32,
                    justAdded: *mut u32,
                ) -> IteratorPointer = std::mem::transmute(cMethod);
                let mut justCreatedValue: u32 = 0;
                let justCreatedPtr = if justCreated.is_some() {
                    &mut justCreatedValue
                } else {
                    nullPtr as *mut u32
                };
                pointer = method(self.array, index, create as u32, justCreatedPtr);
                if let Some(jc) = justCreated {
                    *jc = justCreatedValue != 0;
                }
            }
        }
        pointer
    ...
    pub fn getData(&self, pointer: IteratorPointer) -> T {
        ...
        unsafe {
            ...
            let cMethod: usize =
                std::mem::transmute(*vTbl.add(ecrt_sys::Container_getData_vTblID as usize));
            if cMethod != 0usize {
                let method: unsafe extern "C" fn(
                    container: ecrt_sys::Array,
                    pointer: IteratorPointer,
    ...
    pub fn setData(&mut self, pointer: IteratorPointer, value: &T) -> bool {
        ...
        unsafe {
            ...
            let cMethod: usize =
                std::mem::transmute(*vTbl.add(ecrt_sys::Container_setData_vTblID as usize));
            if cMethod != 0usize {
                let method: unsafe extern "C" fn(
                    container: ecrt_sys::Array,
                    pointer: IteratorPointer,
...
impl Default for String {
    fn default() -> Self {
        String(nullPtr as ecrt_sys::String)
    }
}
...
pub fn tokenizeWith<const MAX_TOKENS: usize>(
    string: &str,
    tokenizers: &str,
    ...
    unsafe {
        ...
        let nTokens: i32 = ecrt_sys::tokenizeWith.unwrap()(
            cString,
            MAX_TOKENS as i32,
            tokensArray.as_mut_ptr(),
            cTokenizers.as_ptr(),
            escapeBackSlashes as u32,
        );
        ...
        for i in 0..nTokens {
            tokens[i as usize] = CStr::from_ptr(tokensArray[i as usize])
                .to_str()
                .unwrap()
                .to_string();
        }
...
impl File {
    pub fn open(fileName: &str, mode: FileOpenMode) -> Result<Self, std::string::String> {
        ...
        unsafe {
            f = ecrt_sys::fileOpen.unwrap()(csFileName.as_ptr(), mode as ecrt_sys::FileOpenMode);
        }
        if f != nullInst {
            Ok(File { file: f })
        } else {
